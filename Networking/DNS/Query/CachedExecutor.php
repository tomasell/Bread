...
class CachedExecutor implements Interfaces\Executor {
  ...
  public function query($nameserver, Query $query) {
    ...
    return $this->cache->lookup($query)->then(function ($cachedRecords) use (
      $that, $query) {
      return $that->buildResponse($query, $cachedRecords);
    }, function () use ($executor, $cache, $nameserver, $query) {
      return $executor->query($nameserver, $query)->then(function ($response) use (
        $cache, $query) {
  ...
  public function buildResponse(Query $query, array $cachedRecords) {
    ...
    $response->questions[] = new Record($query->name, $query->type,
      $query->class);
