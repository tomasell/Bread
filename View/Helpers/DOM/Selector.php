...
class Selector {
  ...
  public static function toXPath($cssExpr, $prefix = 'descendant-or-self::') {
    if (is_string($cssExpr)) {
      ...
      if (preg_match('#^(\w*)\.(\w+)\s*$#u', $cssExpr, $match)) {
        return sprintf("%s%s[contains(concat(' ', normalize-space(@class), ' '), ' %s ')]", $prefix, $match[1] ? $match[1]
          : '*', $match[2]);
      }
  ...
  public function parse($string) {
    $tokenizer = new Selector\Tokenizer();

    $stream = new Selector\TokenStream($tokenizer->tokenize($string), $string);
    ...
    try {
      ...
      throw new $class(
        sprintf('%s at %s -> %s', $e->getMessage(), implode($stream->getUsed(), ''), $stream->peek()),
        0, $e);
    }
  }
  ...
  private function parseSelectorGroup($stream) {
    ...
    return new Selector\Node\OrNode($result);
  }
  ...
  private function parseSelector($stream) {
    ...
    while (true) {
      ...
      $result = new Selector\Node\CombinedSelectorNode($result, $combinator,
        $nextSelector);
    }
  ...
  private function parseSimpleSelector($stream) {
    ...
    $result = new Selector\Node\ElementNode($namespace, $element);
    $hasHash = false;
    while (true) {
      $peek = $stream->peek();
      if ('#' == $peek) {
        ...
        $stream->next();
        $result = new Selector\Node\HashNode($result, $stream->next());
        $hasHash = true;
      ...
      elseif ('.' == $peek) {
        $stream->next();
        $result = new Selector\Node\ClassNode($result, $stream->next());
      ...
      elseif (':' == $peek || '::' == $peek) {
        ...
        if ($stream->peek() == '(') {
          ...
          if (')' != $next) {
            throw new ParseException(
              sprintf("Expected ')', got '%s' and '%s'", $next, $selector));
          }

          $result = new Selector\Node\FunctionNode($result, $type, $ident, $selector);
        }
        else {
          $result = new Selector\Node\PseudoNode($result, $type, $ident);
        }
  ...
  private function parseAttrib($selector, $stream) {
    ...
    if ($stream->peek() == ']') {
      return new Selector\Node\AttribNode($selector, $namespace, $attrib, 'exists', null);
    }
    ...
    return new Selector\Node\AttribNode($selector, $namespace, $attrib, $op, $value);
  }
}
